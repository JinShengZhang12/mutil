...
def _fallback_judge(question_data: Dict[str, Any], student_answer: str) -> Dict[str, Any]:
    ...
    if qid == 1:
        ...
        off_topic = _contains_any(answer, ["不知道", "随便", "不清楚"]) and len(answer) <= 6
        ...
        if off_topic:
            incorrect_points.append("回答过于简单，缺少与题目相关的推理")

        if has_3cm and has_right and has_reason:
            score = 5
        elif has_3cm:
            score = 3
        else:
...
def _fallback_analyzer(
    ...
    if qid == 1:
        message = (
            "请回顾平衡条件：左边的效果要等于右边两个物体效果的总和。"
            "先算左边5×50，再想想右边已经有10×10，还差多少，再决定50kg该放多远。"
        )
    else:
        message = (
            "请先算左边5×50的总效果，再减去右边10×10已经提供的部分。"
            "剩下的效果要由15cm处的新物体提供，所以再想想它应该多重。"
        )
...
def analyzer_agent(
    ...
    try:
        ...
        return _fallback_analyzer(question_data, student_answer, judge_result, attempt_count)
...
def scheduler_agent(
    ...
    reply_lines = [
        f"【本次评分】{judge_result['score']}分",
        f"【点评】{judge_result['brief_comment']}",
    ]
    ...
    if judge_result.get("incorrect_points"):
        reply_lines.append("【需纠正】" + "；".join(judge_result["incorrect_points"][:3]))

    should_finish_question = current_attempt >= 2 or judge_result.get("is_goal_reached")

    if should_finish_question:
        reply_lines.append("【本题总结】判断杠杆平衡时，要比较左边总效果和右边总效果是否相等；多个物体时要先分别算再相加。")
        state = advance_question(state)
        if is_session_finished(state, LESSON_KB):
            state["finished"] = True
            total_score = compute_total_score(state)
            final_summary = build_final_summary(state, LESSON_KB)
            reply_lines.append(f"【总分】{total_score}分")
            reply_lines.append(f"【总评】{final_summary}")
        else:
    ...
    log_all_interactions(state.get("student_id", "unknown"), payload)
    save_student_dialog(state.get("student_id", "unknown"), payload)
    return state, "\n".join(reply_lines)
...
def start_session(student_id: str):
    ...
    status = build_progress_text(state, LESSON_KB)

    payload = {"event": "start_session", "state_snapshot": state}
    log_all_interactions(student_id, payload)
    save_student_dialog(student_id, payload)
    return state, chat_history, status
...
def submit_answer(student_answer: str, chat_history: List[List[str]], state: Optional[Dict[str, Any]]):
    ...
    if is_session_finished(state, LESSON_KB):
        final_text = build_final_summary(state, LESSON_KB)
        return (
            state,
            _append_chat(chat_history or [], student_answer or "（空）", f"已完成全部题目。\n【总评】{final_text}"),
            "已完成",
        )
    ...
    new_history = _append_chat(chat_history or [], answer, bot_reply)
    status = build_progress_text(new_state, LESSON_KB)
    return new_state, new_history, status
...
def load_teacher_logs(password: str):
    try:
        ...
        if isinstance(result, dict):
            ...
            lines = [json.dumps(item, ensure_ascii=False) for item in logs]
...
with gr.Blocks(title="学生科学推理能力辅助系统") as demo:
    ...
    start_btn.click(
        ...
        outputs=[session_state, chatbot, status_text],
    )

    send_btn.click(
        fn=submit_answer,
        inputs=[answer_input, chatbot, session_state],
        outputs=[session_state, chatbot, status_text],
    ...
    clear_btn.click(
        fn=clear_student_ui,
        outputs=[session_state, chatbot, answer_input, status_text],
    )

    load_logs_btn.click(
        fn=load_teacher_logs,
        inputs=[teacher_password],
        outputs=[logs_box],
    )
